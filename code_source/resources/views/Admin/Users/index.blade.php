    ...
    <div class="bg-white rounded-lg border border-gray-200 mx-8 mb-8">
        ...
        <div class="p-4 border-b bg-gray-50 border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div class="flex gap-2">
                <a href="{{ route('Users.index', ['eligibility' => 'eligible']) }}" class="px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm {{ request('eligibility') === 'eligible' ? 'bg-blue-50 border-blue-300' : '' }}">Eligible</a>
                <a href="{{ route('Users.index', ['eligibility' => 'ineligible']) }}" class="px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm {{ request('eligibility') === 'ineligible' ? 'bg-blue-50 border-blue-300' : '' }}">Ineligible</a>
            </div>
        </div>
                        ...
                        <td class="px-6 text-center py-4 relative">
                            ...
                            <div class="dropdown w-48 bg-white rounded-md shadow-lg border border-gray-200">
                                <div class="py-1">
                                    ...
                                    <form action="{{ route('Users.destroy', $user->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this donor?');">
                                        @csrf
                                        @method('DELETE')
